...
struct PBKitImageData {
  ...
  UVRect MakeTexCoords(GPU_Rect* src_rect, GPU_Image* image) const {
    ...
    float h_conversion = (float)image->base_w / (float)image->texture_w;
    float v_conversion = (float)image->base_h / (float)image->texture_h;

    UVRect ret{ pixel_left / (float)image->base_w * h_conversion,
                pixel_top / (float)image->base_h * v_conversion,
                pixel_right / (float)image->base_w * h_conversion,
                pixel_bottom / (float)image->base_h * v_conversion };
    return std::move(ret);
  }
};
...
static void SDLCALL UpdateImage(GPU_Renderer* renderer,
  ...
  swizzle_rect(source, image->texture_w, image->texture_h, image_data->data, source_pitch,
               source_bpp);
...
static void BindTexture(GPU_Image* image, uint32_t stage = 0) {
  ...
  uint32_t format = MASK(NV097_SET_TEXTURE_FORMAT_CONTEXT_DMA, DMA_A)
                    | MASK(NV097_SET_TEXTURE_FORMAT_CUBEMAP_ENABLE, false)
                    | MASK(NV097_SET_TEXTURE_FORMAT_BORDER_SOURCE, 0)
  ...
  p = pb_push2(p, NV20_TCL_PRIMITIVE_3D_TX_OFFSET(stage),
               (intptr_t)image_data->data & 0x03ffffff, format);

  uint32_t pitch_param = (image->texture_w * image->bytes_per_pixel) << 16;
  ...
  p = pb_push1(p, NV20_TCL_PRIMITIVE_3D_TX_FILTER(stage), 0x1012000);

  p = pb_push1(p, NV097_SET_TEXTURE_BORDER_COLOR, 0);
...
static void SDLCALL BlitTransformX(GPU_Renderer* renderer,
  ...
  auto vtx = [&p](float x, float y, float u, float v) {
    p = pb_push4f(p, NV097_SET_DIFFUSE_COLOR4F, 1.0, 0.0, 0.75, 0.75);
...
#ifdef NXDK
  renderer_id = GPU_MakeRendererID("pbkit", GPU_RENDERER_PBKIT, 1, 0);
  GPU_RegisterRenderer(renderer_id, &CreateRenderer, &FreeRenderer);

  GPU_SetRendererOrder(1, &renderer_id);
#endif
}
